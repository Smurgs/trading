...
class SimpleModel(object):
    ...
    def train(self):
        ...
        hour_conv1_1 = layers.Conv1D(30, 50, padding='same')(hour_inputs)
        hour_conv1_2 = layers.Conv1D(30, 21, padding='same')(hour_inputs)
        hour_conv1_3 = layers.Conv1D(30, 11, padding='same')(hour_inputs)
        hour_conv1 = layers.concatenate([hour_conv1_1, hour_conv1_2, hour_conv1_3])
        hour_conv1 = layers.MaxPool1D(10)(hour_conv1)
        hour_conv2_1 = layers.Conv1D(30, 50, padding='same')(hour_conv1)
        hour_conv2_2 = layers.Conv1D(30, 21, padding='same')(hour_conv1)
        hour_conv2_3 = layers.Conv1D(30, 11, padding='same')(hour_conv1)
        hour_conv2 = layers.concatenate([hour_conv2_1, hour_conv2_2, hour_conv2_3])
        hour_conv2 = layers.MaxPool1D(5)(hour_conv2)
        hour_conv3_1 = layers.Conv1D(30, 50, padding='same')(hour_conv2)
        hour_conv3_2 = layers.Conv1D(30, 21, padding='same')(hour_conv2)
        hour_conv3_3 = layers.Conv1D(30, 11, padding='same')(hour_conv2)
        hour_conv3 = layers.concatenate([hour_conv3_1, hour_conv3_2, hour_conv3_3])
        hour_conv3 = layers.MaxPool1D(5)(hour_conv3)
        hour_flat1 = layers.Flatten()(hour_conv3)
        hour_dense1 = layers.Dense(512, activation='softmax')(hour_flat1)
        hour_dense2 = layers.Dense(512, activation='softmax')(hour_dense1)

        day_inputs = Input(shape=(250, 4), name='day_inputs')
        day_conv1_1 = layers.Conv1D(30, 50, padding='same')(day_inputs)
        day_conv1_2 = layers.Conv1D(30, 21, padding='same')(day_inputs)
        day_conv1_3 = layers.Conv1D(30, 11, padding='same')(day_inputs)
        day_conv1 = layers.concatenate([day_conv1_1, day_conv1_2, day_conv1_3])
        day_conv1 = layers.MaxPool1D(10)(day_conv1)
        day_conv2_1 = layers.Conv1D(30, 50, padding='same')(day_conv1)
        day_conv2_2 = layers.Conv1D(30, 21, padding='same')(day_conv1)
        day_conv2_3 = layers.Conv1D(30, 11, padding='same')(day_conv1)
        day_conv2 = layers.concatenate([day_conv2_1, day_conv2_2, day_conv2_3])
        day_conv2 = layers.MaxPool1D(5)(day_conv2)
        day_conv3_1 = layers.Conv1D(30, 50, padding='same')(day_conv2)
        day_conv3_2 = layers.Conv1D(30, 21, padding='same')(day_conv2)
        day_conv3_3 = layers.Conv1D(30, 11, padding='same')(day_conv2)
        day_conv3 = layers.concatenate([day_conv3_1, day_conv3_2, day_conv3_3])
        day_conv3 = layers.MaxPool1D(5)(day_conv3)
        day_flat1 = layers.Flatten()(day_conv3)
        day_dense1 = layers.Dense(512, activation='softmax')(day_flat1)
        day_dense2 = layers.Dense(512, activation='softmax')(day_dense1)

        merged_concat = layers.concatenate([min_dense2, hour_dense2, day_dense2])
        merged_dense1 = layers.Dense(1024, activation='softmax')(merged_concat)
        merged_dense2 = layers.Dense(1024, activation='softmax')(merged_dense1)
        ...
        model.fit(np.split(x, 3, axis=-1), one_hot_labels, epochs=3, batch_size=32)

        predictions = model.predict(np.split(x, 3, axis=-1))
